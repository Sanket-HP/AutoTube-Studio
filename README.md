...
Voice Studio: High-fidelity text-to-speech synthesis with support for multiple personas (Male/Female) and custom target languages (Hindi, Marathi, French, Spanish, etc.).

Asset Forge: Real-time generation of cinematic 16:9 visual frames tailored to each scene.

Mastering Engine: A client-side rendering pipeline that encodes high-bitrate WebM video files directly from session memory.
...
The codebase, architectural logic, and proprietary synthesis workflows are the intellectual property of the developer.

## 🛠️ Technology Stack

Frontend: HTML5 Canvas, Tailwind CSS, Plus Jakarta Sans Typography.

AI Engine: Google Gemini (2.5 Flash for Logic, TTS for Audio, Imagen 4.0 for Visuals).

Backend/Auth: Firebase Authentication & Firestore (Strict Pathing Architecture).
